...
export const usePurchaseProfile = () => {
  ...
  const onSelectColor = useCallback(
    (color: string) => {
      ...
      updateProfileInfo({ color: upperCaseColor });
    },
    [updateProfileInfo]
  );
  ...
  const merchantAttributes = useMemo(
    () => ({
  ...
  const fakeTestPurchase = useCallback(async () => {
    // TEST Only (this will be called on currentPurchase effect)
    const { data, error } = await profilePurchaseHubQuery({
      iapReceipt: testReceipt,
      provider: testProvider,
      merchantDID: merchantAttributes,
    });
